...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    require_csrf('/faculty/notifications.php');
    $action = $_POST['action'] ?? '';
    ...
    if ($action === 'mark_read') {
        ...
        try {
            $pdo->prepare('UPDATE notifications SET is_read = 1, read_at = NOW() WHERE id = ? AND user_id = ?')->execute([$notificationId, $userId]);
            flash('success', 'Notification marked as read.');
        } catch (Throwable $e) {
            error_log('faculty notifications mark_read: ' . $e->getMessage());
            flash('error', 'Something went wrong. Please try again.');
        }
        redirect('/faculty/notifications.php' . ($backPage > 1 ? '?page=' . $backPage : ''));
    }
    ...
    if ($action === 'mark_all_read') {
        try {
            ...
            error_log('faculty notifications mark_all_read: ' . $e->getMessage());
            flash('error', 'Something went wrong. Please try again.');
        }
        redirect('/faculty/notifications.php');
    }
    redirect('/faculty/notifications.php');
}
    ...
    <main class="dashboard-main">
        ...
        <section class="dashboard-section">
            <div class="section-title-row">
                <h2>Notifications</h2>
                <?php if ($totalCount > 0): ?>
                    <form method="post" action="<?= e(url('/faculty/notifications.php')) ?>" class="m-0">
                        <?= csrf_field() ?>
                        <input type="hidden" name="action" value="mark_all_read">
                        <button type="submit" class="btn btn-sm btn-outline-primary"><i class="bi bi-check2-all"></i> Mark all as read</button>
                    </form>
                <?php endif; ?>
            </div>

            <?php if (!$notifications): ?>
                <div class="app-empty-state"><i class="bi bi-bell"></i><p>You have no notifications yet.</p></div>
            <?php else: ?>
                <div class="list-group">
                    <?php foreach ($notifications as $n): ?>
                        <?php
                            ...
                            $link     = notification_link($n['related_type'], $n['related_id'] !== null ? (int)$n['related_id'] : null, 'faculty');
                        ?>
                        <div class="list-group-item d-flex align-items-start gap-3 py-3 <?= $isUnread ? 'fw-bold border-start border-3' : '' ?>" <?= $isUnread ? 'style="border-color:#064b9b !important;"' : '' ?>>
                            <div class="stat-card-icon blue flex-shrink-0" style="width:42px;height:42px;font-size:18px;"><i class="bi <?= e($icon) ?>"></i></div>
                                ...
                                <div class="d-flex align-items-center gap-3">
                                    ...
                                    <?php if ($isUnread): ?>
                                        <form method="post" action="<?= e(url('/faculty/notifications.php')) ?>" class="m-0">
                                            ...
                                            <button type="submit" class="btn btn-sm btn-link p-0 small fw-normal text-decoration-none"><i class="bi bi-check2"></i> Mark as read</button>
                                        </form>
                                    <?php endif; ?>
                                </div>
                            </div>
                ...
                <?php if ($totalPages > 1): ?>
                    <nav class="app-pagination" aria-label="Notifications pagination">
                        <?php if ($page > 1): ?><a href="<?= e(page_url($page - 1)) ?>"><i class="bi bi-chevron-left"></i></a><?php endif; ?>
                        <?php for ($p = 1; $p <= $totalPages; $p++): ?>
                            <?php if ($p === $page): ?><span class="active"><?= $p ?></span><?php else: ?><a href="<?= e(page_url($p)) ?>"><?= $p ?></a><?php endif; ?>
                        <?php endfor; ?>
                        <?php if ($page < $totalPages): ?><a href="<?= e(page_url($page + 1)) ?>"><i class="bi bi-chevron-right"></i></a><?php endif; ?>
                    </nav>
                <?php endif; ?>
            <?php endif; ?>
        </section>
