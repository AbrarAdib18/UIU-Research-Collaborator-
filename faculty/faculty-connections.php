...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    require_csrf('/faculty/faculty-connections.php');
    $action = $_POST['action'] ?? '';
    ...
    if ($action === 'send') {
        ...
        if ($recipientId <= 0 || $recipientId === $userId) {
            ...
            try {
                ...
                error_log('faculty-connections send: ' . $ex->getMessage());
        ...
        if (!$conn) {
            ...
            try {
                ...
                error_log('faculty-connections ' . $action . ': ' . $ex->getMessage());
                flash('error', 'Could not update connection request.');
            }
        }
    }
    redirect('/faculty/faculty-connections.php');
}

$incoming = $pdo->prepare(
    "SELECT rc.*, u.name, u.role FROM research_connections rc JOIN users u ON u.id = rc.requester_id
     WHERE rc.recipient_id = ? AND rc.status = 'pending' ORDER BY rc.created_at DESC"
);
$incoming->execute([$userId]);
$incoming = $incoming->fetchAll();

$outgoing = $pdo->prepare(
    "SELECT rc.*, u.name, u.role FROM research_connections rc JOIN users u ON u.id = rc.recipient_id
     WHERE rc.requester_id = ? AND rc.status = 'pending' ORDER BY rc.created_at DESC"
);
...
<?php require __DIR__ . '/../includes/faculty_sidebar.php'; ?>
    <main class="dashboard-main">
        <?php render_flashes(); ?>
        <h2 style="color:var(--uiu-blue);font-size:23px;font-weight:700;">Connections</h2>
        ...
        <div class="app-panel">
            ...
            <?php foreach ($accepted as $c): ?>
                <div class="conn-row">
                    ...
                    <a href="<?= e(url('/faculty/conversation.php?user=' . $c['other_id'])) ?>" class="btn btn-sm btn-outline-primary"><i class="bi bi-chat-dots"></i> Message</a>
                </div>
            <?php endforeach; ?>
        </div>
    </main>
